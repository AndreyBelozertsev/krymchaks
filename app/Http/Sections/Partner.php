 ...
 * Class Partner
 *
 * @property \App\Models\Partner $model
 *
...
class Partner extends Section implements Initializable
{
    ...
    {
        $this->addToNavigation()->setPriority(600)->setIcon('fa fa-handshake');
    }
    ...
    public function onDisplay($payload = [])
    {
        $columns = [
            ...
            AdminColumn::text('sort', 'Порядок сортировки')
                ->setSearchable(false),
            AdminColumn::text('created_at', 'Дата добавления / редактирования', 'updated_at')
                ->setWidth('160px')
                ->setOrderable(function($query, $direction) {
    ...
    public function onEdit($id = null, $payload = [])
    {
        $form = AdminForm::card()->addBody([
            ...
            AdminFormElement::text('title', 'Название')
                ->required(),
            AdminFormElement::image('thumbnail', 'Лого')
                ->setUploadPath(function($file) {
                    return PathSaveClass::getUploadPath('partners','images'); 
                }),
            AdminFormElement::text('url', 'Ссылка на партнера'),
